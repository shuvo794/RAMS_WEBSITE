...
// Custom dropdown option with flag
...
const CustomSingleValue = ({ data }: { data: CountryOption }) => {
  ...
  // During SSR and before hydration, show desktop version to match server
  if (!isMounted) {
    return (
      <div
        style={{
      ...
      >
        ...
        />
        <span>{`${data.code} ${data.value}`}</span>
      </div>
    );
  }

  // After hydration, show responsive version
  return (
    <div
      style={{
        display: "flex",
    ...
    >
      ...
      <span>{isMobile ? data.value : `${data.code} ${data.value}`}</span>
    </div>
  );
};
    ...
    <Controller
      ...
      render={({ field }) => (
        <Select
          ...
          getOptionLabel={(e) => e.value || e.code}
          getOptionValue={(e) => e.value || e.code}
